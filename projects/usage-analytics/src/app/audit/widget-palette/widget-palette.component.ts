...
export class WidgetPaletteComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        /** Update palette after each dashboard change */
        this._dashboardChangesSubscription = this.dashboardService.dashboardChanged.subscribe(() => this.palette = this.getPalette());

        /** Update palette after query change ( open new dashboard, shared dashboard ...) */
        this._querySubscription = this.searchService.queryStream.subscribe(() => this.palette = this.getPalette());
    }
    ...
    getPalette(): {name: string, items: DashboardItemOption[]}[] {
        return this.dashboardService.getPalette().map(
            (p: {name: string, items: DashboardItemOption[]}) => ({
                name: p.name,
                items: p.items.filter((widget) => !this.dashboardService.dashboard.items.find((item) => item.title === widget.text))
            })
        )
    }
